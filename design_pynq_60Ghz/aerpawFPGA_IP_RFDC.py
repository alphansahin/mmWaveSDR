...
with open(os.path.join(_THIS_DIR, 'aerpawFPGA_IP_RFDC_functions.c'), 'r') as f: header_text = f.read()
_ffi = cffi.FFI()
_ffi.cdef(header_text)

_lib = _ffi.dlopen(os.path.join(_THIS_DIR, 'aerpawFPGA_IP_RFDC_libxrfdc.so'))
...
class RFdc(DefaultIP):
    ...
    def __init__(self, description):
        ...
        print('RF Data Converter initilization starts...')
        if 'parameters' in description:
            from aerpawFPGA_IP_RFDC_config import populate_config
        ...
        self.myIPStatus = _ffi.new("XRFdc_IPStatus*")     
        status = _lib.XRFdc_GetIPStatus(self._instance,self.myIPStatus)
        if status != 0:
            print('IP didn''t start...')
        else:
            if False:
                for i in range(4):
                    print("ADC Tile " + str(i));
                    print("> PowerUp Status: " + str(self.myIPStatus.ADCTileStatus[i].PowerUpState))
                    print("> Tile State: " + str(self.myIPStatus.ADCTileStatus[i].TileState))
                    print("> PLL State: " + str(self.myIPStatus.ADCTileStatus[i].PLLState))   
                    print("> IsEnabled: " + str(self.myIPStatus.ADCTileStatus[i].IsEnabled))  
                    print("> BlockStatusMask: " + str(self.myIPStatus.ADCTileStatus[i].BlockStatusMask))  

                for i in range(4):
                    print("DAC Tile " + str(i));
                    print("> PowerUp Status: " + str(self.myIPStatus.DACTileStatus[i].PowerUpState))
                    print("> Tile State: " + str(self.myIPStatus.DACTileStatus[i].TileState))
                    print("> PLL State: " + str(self.myIPStatus.DACTileStatus[i].PLLState))   
                    print("> IsEnabled: " + str(self.myIPStatus.DACTileStatus[i].IsEnabled))  
                    print("> BlockStatusMask: " + str(self.myIPStatus.DACTileStatus[i].BlockStatusMask))  
    ...
    def runMTS(self):
        ...
        _lib.XRFdc_MultiConverter_Init(self._ADC_Sync_Config,_ffi.new('int*'),_ffi.new('int*'))
